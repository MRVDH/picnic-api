...
const checkout_error_1 = require("./errors/checkout-error");
/**
...
class HttpClient {
    ...
    sendRequest(method, path, data = null, includePicnicHeaders = false, isImageRequest = false) {
        ...
        return this.performRequest(method, path, headers, data ? JSON.stringify(data) : null, isImageRequest);
    }
    /**
     ...
     * @param {boolean} [includePicnicHeaders=false] Whether to include x-picnic-agent and x-picnic-did headers.
     */
    sendRawRequest(method, path, body, contentType, includePicnicHeaders = false) {
        const headers = new Headers({
            ...this.baseHeaders,
            ...(includePicnicHeaders && this.picnicHeaders),
            "Content-Type": contentType,
        });
        return this.performRequest(method, path, headers, body, false);
    }
    /**
     ...
     */
    async performRequest(method, path, headers, body, isImageRequest) {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                const checkoutIssue = (0, checkout_error_1.parseCheckoutIssueError)(errorData);
                if (checkoutIssue)
                    throw checkoutIssue;
                const parsed = errorData;
