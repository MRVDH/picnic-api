            ...
            for (const child2 of child1.children) {
                if (child2.content?.selling_unit) {
                    try {
                        ...
                        throw new Error("Failed to parse the search results and find the sole_article_id. This could mean that the response format from the picnic servers has changed. Please open an issue on the github repository.");
                    }
                }
            }
        }
