...
- Some pages are served as a React Server Components (RSC) payload (`text/x-component`) instead of a Fusion page, depending on the page and the `x-picnic-agent` version. Newer app versions get RSC for pages that Picnic moved to its newer page platform (known: `category-tree-root`, `profile-root`, `promo-group-deep-dive`). `app.getPage` throws an `UnexpectedPageFormatError` for those; fetch them with `app.getRscPage`, which parses the payload into rows (`src/domains/app/rsc.ts`, types in `src/types/rsc.ts`). When bumping the default agent in `src/http-client.ts`, check which pages switch format.
